...
let deliveryStage = 1;
const stages = ['tls1','tls2','tls3','tls4'];
function renderTimeline(){
  ...
  document.getElementById('tlFill').style.width = (deliveryStage/3*100)+'%';
  const slot = document.getElementById('photoSlot');
  if(deliveryStage>=3){
    slot.classList.add('filled');
    slot.innerHTML = '✓ Delivered at 1:04 PM — photo uploaded by driver';
  } else if(deliveryStage===2){
    slot.classList.remove('filled');
    slot.innerHTML = '🛵 Driver is on the way — you\'ll get a notification when it arrives';
  } else {
    slot.classList.remove('filled');
    slot.innerHTML = 'Delivery photo will appear here once our driver marks it delivered';
  }
}
window.setDeliveryStage = function(stage){
  deliveryStage = Math.max(0, Math.min(3, stage));
  renderTimeline();
};
function advanceDelivery(){
  if(deliveryStage < 3){ deliveryStage++; renderTimeline();
    if(deliveryStage===2) toast('🛵 Your tiffin is on the way!');
    if(deliveryStage===3) toast('🏠 Delivered! Photo uploaded to your dashboard.');
  } else {
    toast('Already delivered for today ✓');
  }
}
...
function buildCalendar(){
  ...
  if(!grid) return;
  const today = 2, daysInMonth = 31;
  const dow = ['S','M','T','W','T','F','S'];
  dow.forEach(d=>{ const el=document.createElement('div'); el.className='d'; el.style.background='transparent'; el.style.border='none'; el.style.color='var(--pencil)'; el.textContent=d; grid.appendChild(el); });
  const firstDow = 6; // Aug 1 2026 = Saturday
  ...
  for(let d=1; d<=daysInMonth; d++){
    ...
    el.className = 'd' + (d<today? ' done': d===today? ' today':' pending');
    el.textContent = d;
    grid.appendChild(el);
  }
}
